...
Doctor:"""
    ...
    def setup_doctor_game(self, session_id: str) -> Dict[str, Any]:
        ...
        # Set up game state
        game_state.hidden_condition = case_data["diagnosis"]["name"]
        game_state.condition_data = case_data
        game_state.patient_name = self._generate_patient_name()
        game_state.patient_age = random.randint(
            case_data.get("age_range", {}).get("min", 25),
            case_data.get("age_range", {}).get("max", 65)
        )
    ...
    async def _generate_medical_case_async(self, difficulty: str, 
        ...
        # For now, return a simple case structure
        return {
            "diagnosis": {"name": "Generated Condition"},
            "chief_complaint": "Generated complaint",
            "symptoms": ["symptom1", "symptom2"],
            "medical_history": "Generated history",
    ...
    def submit_diagnosis(self, session_id: str, diagnosis: str) -> Dict[str, Any]:
        ...
        if correct:
            ...
            return {
                ...
                "condition_info": game_state.condition_data.get("diagnosis", {})
            }
        else:
            self._save_session(session_id, game_state)
